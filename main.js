...
function openDual(exterior, interior){
  let m = document.getElementById('modal-global');
  if(!m){
    m = document.createElement('div'); m.id='modal-global'; m.style.cssText='position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.9);z-index:9999;padding:20px';
    m.innerHTML = '<div style="max-width:96%;max-height:96%;overflow:auto;background:#0b0b0b;padding:12px;border-radius:10px;position:relative"><button onclick="closeModal()" style="position:absolute;right:8px;top:8px;background:#fff;border-radius:6px;padding:6px;cursor:pointer">✕</button><h3 style="color:#ff8c4d">Exterior (2560x1440)</h3><img class="ext" src="" style="max-width:100%"><h3 style="color:#ff8c4d">Interior (728x486)</h3><img class="int" src="" style="max-width:100%"></div>';
    document.body.appendChild(m);
  }
  m.querySelector('.ext').src = exterior; m.querySelector('.int').src = interior; m.style.display='flex';
}
function generateContent(){
  document.querySelectorAll('[data-gen="grid"]').forEach(container=>{
    const count = parseInt(container.getAttribute('data-count')||'0',10);
    const type = container.getAttribute('data-type')||'cell';
    container.innerHTML = '';
    for(let i=1;i<=count;i++){
      if(type === '80'){
        const el = document.createElement('div'); el.className='cell'; el.innerHTML=`<img src="/assets/images/placeholder_80.png" alt="NUMERO ${i}"><div class="num">NUMERO ${i}</div>`; el.onclick = ()=> openModal('/assets/images/placeholder_80.png'); container.appendChild(el);
      } else if(type === 'box'){
        const el = document.createElement('div'); el.className='box'; el.innerHTML=`<img src="/assets/images/placeholder_280.png" style="width:100%;height:100%;object-fit:cover"><div class="num">NUMERO ${i}</div>`; el.onclick = ()=> openModal('/assets/images/placeholder_280.png'); container.appendChild(el);
      } else if(type === 'swiper'){
        const slide = document.createElement('div'); slide.className='swiper-slide'; slide.innerHTML=`<img src="/assets/images/placeholder_pc.png" alt="NUMERO ${i}"><div class="slide-num">NUMERO ${i}</div>`; slide.onclick = ()=> openModal('/assets/images/placeholder_pc.png'); container.appendChild(slide);
      }
    }
  });
}
document.addEventListener('DOMContentLoaded', ()=>{ generateContent(); initSwipers(); });
